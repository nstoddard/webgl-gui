...
impl Color4 {
    ...
    /// Converts the `Color4` to an array.
    ...
    /// sRGB, this will typically result in a more realistic-looking result than multiplying in
...
impl Color4Uniform {
    ...
    /// Sets the uniform. If `convert_to_srgb` is true, the color will be converted to sRGB
    /// first. In most cases, the color should be kept in a linear color space here
    /// (so `convert_to_srgb` would be false), and the
    /// fragment shader should convert to sRGB as the final step.
